...
and ``tailnet_mobile._guard`` closes on the QR surface). Enforced here in two
halves: restricted sessions are refused outright, and the caller's own token
bounds (``boot``, ``no_refresh``, remaining ``session_exp``) are carried into
the minted link so the new session inherits — never exceeds — them.
"""
...
def _caller_bounds(request: web.Request) -> tuple[dict[str, str], int]:
    ...
    Returns ``(carried_claims, ttl_ceiling_seconds)``. Claims are carried, never
    re-derived: ``boot`` copied verbatim (same rule as the link→session exchange
    in ``token_auth``), ``no_refresh`` copied so the recipient session never
    grows a refresh chain, and the remaining ``session_exp`` becomes the TTL
    ceiling so a short-lived caller cannot mint a longer-lived credential.
    Fail-closed on an unreadable payload: a caller whose bounds cannot be
    established gets a bounded (no-refresh, default-TTL-capped) link rather than
    an unbounded one.
    ...
    **Extraction order must mirror the middleware's, query param before cookie.**
    ...
    try:
        ...
        if session_exp:
            ...
            ttl_ceiling = max(1, min(ttl_ceiling, remaining))
    except Exception:
        return {"no_refresh": "1"}, ttl_ceiling
...
async def api_auth_mobile_link(request: web.Request) -> web.Response:
    ...
    if state is not None and _is_restricted_session(state, request):
        _audit(user_id, "restricted_session_denied")
        return web.json_response(
