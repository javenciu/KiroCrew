...
async def _handle_drive_upload(request: web.Request) -> web.Response:
    ...
    try:
        ...
        # the transfer may no longer hold. Re-check the cheap, local decisions
        # (app still enabled, consent still granted) immediately before the
        # bytes are committed to S3.
        if not await asyncio.to_thread(is_app_enabled, APP_NAME):
