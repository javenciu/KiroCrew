...
* `_restore_logger_configuration` puts every logger's level, `disabled` flag and
  handlers back. A level is process-global AND hierarchical, so an explicit one left on
  `kiro_crew` decides what every `kiro_crew.*` logger in the worker may emit and it
  outranks the root level `caplog.at_level()` sets — the victim's `caplog.text` comes
  back **empty**, not wrong, which reads as "the code stopped logging" rather than as
  pollution. `cli._setup_cli_logging` pins `kiro_crew` at WARNING and attaches a rotating
  file handler, and test modules across the suite run it for real by driving `cli.main()`
  in process, so the handlers accumulate too — each still open on a `gateway.log` under a
  `tmp_path` the next test deletes. Restored rather than blamed, for the same reason the
  CWD restore is. The root logger's **handler list** is deliberately excluded (its level
  is not): pytest's own `catching_logs` adds one handler per test phase and removes it at
  the phase boundary, so writing back a list snapshotted during setup would re-attach the
  setup phase's handler and drop the one the teardown phase is capturing through.
