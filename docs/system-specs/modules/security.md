...
- **Doctest examples are code, on both source-subject passes** (`_doctest_example_subjects`): `doctest` EXECUTES the `>>> ` examples inside a docstring, but to every scan above a docstring is one opaque string — its Python-call spelling (`>>> subprocess.run("grep -r secret …", shell=True)`) is not shell grammar, and its escape spelling (`\x25LOCALAPPDATA\x25…`) matches no fence pattern as raw text while the example's OWN parse decodes it into a read of the real store. So every example is re-read with Python's own parser (`doctest.DocTestParser`, the same PS1/PS2 grammar the runner uses, never a hand-rolled regex) and the string literals inside the example code are extracted, decoded exactly as the example would decode them. They join BOTH consumers: each rides `_source_command_subjects` as its own subject under the SAME `_SOURCE_COMMAND_SUBJECT_CAP` (a body over the cap was never inspected, so allowing would make the cap the bypass), and each takes the fence check inside `_sensitive_run_in_source_literals` — wired at the constant, after that literal's own raw text is checked, so the two layers cannot disagree about which strings exist. The pattern-slot exoneration is NOT inherited by an extracted literal: the exoneration is earned by the literal that OCCUPIES the slot, and an example literal sits inside that literal rather than in the slot, so it denies — the direction every doubtful exoneration takes here. Every fallback errs toward denying, matching the module's direction everywhere else: example code that is not valid Python (a shell transcript pasted after `>>> `) comes back as raw text, a subject that only ADDS a chance to convict; a malformed doctest directive (`ValueError` from the parser) must not exonerate the text, so each contiguous `>>> `/`... ` block goes through the same literal extraction and breaking the doctest syntax cannot buy an exemption for any line of the example. Nested examples (a docstring quoting a docstring) are followed to a small fixed depth; past it the inner text was already appended as a plain subject one level up, so nothing is silently dropped. A value carrying no `>>> ` marker returns immediately, so constants without examples pay a substring test and nothing more.
