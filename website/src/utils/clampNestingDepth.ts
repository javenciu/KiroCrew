 ...
 *  A marker at end of line is an EMPTY item and nests like any other, so it
 *  must match here exactly as it does in CONTAINER_UNIT. */
 ...
 *  Derived from the names table so the two cannot drift apart. */
const HTML_RAW_CLOSERS = [...HTML_RAW_NAMES].map((name) => `</${name}>`)
...
/**
 * Tag-name continuation for the context the line is parsed in. Inline HTML
 * is judged by micromark's grammar (`[A-Za-z][A-Za-z0-9-]*`): anything else
 * makes the whole `<...>` literal text, so parse5 never sees a tag. Raw
 * HTML-BLOCK content goes straight to parse5, whose tag-name state runs until
 * whitespace, '/' or '>' -- so `<x:y>` is a real element there, and reading it
 * with the inline grammar would leave every such open uncounted.
 */
function isTagNameChar(c: number, raw: boolean): boolean {
  if (!raw) return isNameChar(c)
  // NUL is a name byte to the tokenizer (it substitutes U+FFFD and keeps
  // going), so it must not end the name here either.
  return !isHtmlWhitespace(c) && c !== CH_SLASH && c !== CH_GT
}
/** The HTML tokenizer's whitespace set: tab, LF, FF, CR, space. Wider than
 *  CommonMark's (which has no FF or CR), and it must be the set used wherever
 *  raw content is judged, or a tag ended by FF is read as literal text. */
...
interface HtmlScanState {
  ...
  tagQuote: number
  /** Length of the backtick run that opened a still-unclosed inline code
   *  span (0 = none). Carried across lines within a paragraph: CommonMark
   *  closes a span only on a run of EXACTLY this length, so a run of any other
   *  length inside it is literal text and leaves the mask in place. */
  spanRun: number
  /** Kind of opaque span still open across lines (BOGUS_NONE outside one).
   *  parse5 makes such a span a single node, so its body holds no tags and
   *  cannot move the open-element stack. */
  bogusKind: number
}
...
/** Kinds of span whose body parse5 treats as opaque content, so no tag inside
 *  one may move the open-element stack. */
const BOGUS_NONE = 0
/** `<!--`: ends at '-->' or '--!>'. */
const BOGUS_COMMENT = 1
/** Bogus comment: ends at the FIRST '>'. */
const BOGUS_GT = 2

/**
 * Which opaque span starts at `at` (which holds '<'), per the HTML tokenizer.
 * Only `<!--` is a real comment; `<?`, any other `<!`, and `</` followed by a
 * non-name are all BOGUS COMMENTS ending at the first '>'. In particular
 * `<![CDATA[` is one of those: the CDATA state is reachable only inside
 * foreign (SVG/MathML) content, which this model does not enter, so treating
 * it as ending at ']]>' would run the span PAST the '>' where parse5 ends it
 * and skip tags the parser does nest.
 */
function bogusSpanKind(line: string, at: number): number {
  const c1 = line.charCodeAt(at + 1)
  if (c1 === CH_QUESTION || c1 === CH_SLASH) return BOGUS_GT
  if (c1 !== CH_BANG) return BOGUS_NONE
  return line.startsWith('<!--', at) ? BOGUS_COMMENT : BOGUS_GT
}

/**
 * Index just past the end of a `kind` span, searching from `from`, or -1 when
 * the span runs past the end of this line. Callers pass `from` two characters
 * into the opener, which is what makes the abrupt empty-comment spellings
 * (`<!-->`, `<!--->`) end where the tokenizer ends them; ending a span EARLY
 * is the safe direction, since it only puts more tags back under the count.
 */
function bogusSpanEnd(line: string, from: number, kind: number): number {
  if (kind === BOGUS_COMMENT) {
    const plain = line.indexOf('-->', from)
    const bang = line.indexOf('--!>', from)
    if (plain === -1 && bang === -1) return -1
    if (bang === -1 || (plain !== -1 && plain <= bang)) return plain + 3
    return bang + 4
  }
  const gt = line.indexOf('>', from)
  return gt === -1 ? -1 : gt + 1
}

/**
 * Consume the opaque span opening at `at`, returning the next scan index. A
 * span whose end is not on this line latches onto `st` and resumes on the
 * next. A masked opener is literal text inside an inline code span and opens
 * no node at all, so it is stepped over rather than latched: latching there
 * would skip the real tags after the span and UNDER-count.
 */
function takeBogusSpan(
  line: string,
  at: number,
  st: HtmlScanState,
  kind: number,
  ...
  if (masked) return at + 1
  const end = bogusSpanEnd(line, at + 2, kind)
  if (end !== -1) return end
  st.bogusKind = kind
  return line.length
}
...
function clampHtmlLine(line: string, st: HtmlScanState, maxDepth: number, raw: boolean): string {
  ...
  let i = 0
  const n = line.length
  // Inline code-span mask (`st.spanRun`): pair equal-length backtick runs left
  // to right, across lines within a paragraph. The mask is OVER-COUNT-ONLY:
  // while masked, closes never pop (a `</div>` inside a real span is literal
  // to parse5, so popping would under-count) and opens still push. It never
  // decides whether an open past the bound is rewritten -- see below.

  while (i < n) {
    const c = line.charCodeAt(i)
    // Resume an opaque span from a previous line: its body holds no tags, so
    // skip to its end (or consume the whole line and stay latched).
    if (st.bogusKind !== BOGUS_NONE) {
      const end = bogusSpanEnd(line, 0, st.bogusKind)
      if (end === -1) break
      i = end
      st.bogusKind = BOGUS_NONE
      continue
    }
    ...
    if (c === CH_BACKTICK) {
      ...
      if (st.spanRun === 0) {
        st.spanRun = runLen // open a span
      } else if (runLen === st.spanRun) {
        st.spanRun = 0 // close it: equal-length run, per CommonMark
      }
      // else: a run of another length inside the span is literal text
      i += runLen
      continue
    }
    const masked = !raw && st.spanRun !== 0
    if (c !== CH_LT) {
      i++
      continue
    }
    ...
    if (c1 === CH_SLASH) {
      ...
      if (j < n && isNameStart(line.charCodeAt(j))) {
        let k = j + 1
        while (k < n && isTagNameChar(line.charCodeAt(k), raw)) k++
        let e = k
      ...
      // Not a close tag after all. '</' + a non-name is a BOGUS COMMENT to the
      // tokenizer, running to the first '>', so a close tag spelled inside it
      // is content and must not pop either.
      i = takeBogusSpan(line, i, st, BOGUS_GT, masked)
      continue
    }
    if (!isNameStart(c1)) {
      // A comment, declaration or processing instruction is ONE node to parse5:
      // its body is content, so a close tag spelled inside it must not pop (a
      // fake-close bypass) and an open tag inside it must not push.
      const kind = bogusSpanKind(line, i)
      if (kind === BOGUS_NONE) {
        i++ // autolink or stray '<': never a start tag shape
        continue
      }
      i = takeBogusSpan(line, i, st, kind, masked)
      continue
    }
    ...
    // line ending inside the tag); anything else (e.g. ':' outside a raw
    // block) is not a tag to micromark.
    const after = k < n ? line.charCodeAt(k) : 0
    const endsName = after === CH_GT || after === CH_SLASH || (raw ? isHtmlWhitespace(after) : after === 32 || after === 9)
    if (k < n && !endsName) {
      i++
      continue
    }
    ...
    if (st.stack.length >= maxDepth) {
      ...
      // both micromark and parse5 contexts, so the tag cannot parse. This
      // happens whether or not the mask is on: the mask is a GUESS that a
      // span is open, and an unpaired backtick run is literal text to
      // micromark, so honoring the guess here would let one stray backtick
      // switch the guard off for the rest of the paragraph. The cost of
      // being wrong the other way is cosmetic -- `&lt;div>` shown inside a
      // code span that already sits past a hundred open tags.
      out += line.slice(emitted, i) + '&lt;'
      emitted = i + 1
      // Not pushed (it is text now). If the tag was unterminated, the latch
      // above is WRONG for a neutralized tag (its `>` is plain text now) --
      // clear it: subsequent text scans normally.
      if (!closed) {
        st.inTag = false
        st.tagQuote = 0
      }
...
export function clampNestingDepth(s: string): string {
  ...
  const html: HtmlScanState = {
    stack: [],
    inTag: false,
    tagQuote: 0,
    spanRun: 0,
    bogusKind: BOGUS_NONE,
  }
  for (let i = 0; i < lines.length; i++) {
    ...
    if (htmlBlock !== HTML_BLOCK_NONE) {
      ...
      let cur = line
      if (html.inTag || html.bogusKind !== BOGUS_NONE || cur.indexOf('<') >= 0) {
    ...
    if (inFence) continue
    // A blank line ends the paragraph. A code span cannot cross that, so a
    // span still open here never closes: its opener is literal text and the
    // mask it set must not carry into the next paragraph. (A fence line does
    // not reach here, and an HTML-block opener resets below.)
    if (html.spanRun !== 0 && /^[ \t\r]*$/.test(line)) html.spanRun = 0
    ...
    if (cur.indexOf('<') >= 0) {
      ...
      if (kind !== HTML_BLOCK_NONE) {
        rawLine = true
        // Block boundary: an open span belonged to the paragraph that just
        // ended (its opener is literal text) -- it cannot mask block content.
        html.spanRun = 0
