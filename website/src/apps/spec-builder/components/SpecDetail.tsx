...
export default function SpecDetail({ name, setErr, onDeleted, onDuplicated }: SpecDetailProps) {
  ...
  const deleteMutation = useMutation({
    mutationFn: () => specApi.remove(name, specId()),
    onError: (e) => setErr((e as Error).message),
    onSuccess: () => {
  ...
  const sendMessage = messageMutation.mutateAsync
  ...
  return (
    <div ref={bodyRef} className={`flex flex-1 min-w-0 min-h-0 ${isMobile ? 'flex-col' : ''}`}>
          ...
          {hasIdentity && titleDraft === null && dupDraft === null && (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="min-w-[190px]">
                  ...
                  >
                    <Trash2 size={13} className="shrink-0 text-danger" />
      ...
      {confirmDelete && (
        <Modal
          open
          onClose={() => { if (!deleteMutation.isPending) setConfirmDelete(false) }}
            ...
            <>
              ...
              <Btn
                label={deleteMutation.isPending
                  ...
                  : i18nT('apps.specBuilder.components.specDetail.remove_spec', { name })}
                danger
                disabled={deleteMutation.isPending}
        ...
        >
          <p className="text-[13px] leading-relaxed text-text m-0">
