...
function makeDeps({ appVersion = "1.0.0", withNative = true } = {}) {
  const calls = { quitAndInstall: [], notifications: [], exit: 0 };
  const handlers = {};
...
test("Windows uses visible native progress and leaves a recovery notification", async () => {
  ...
  assert.match(calls.notifications[0].body, /several minutes/i);
  assert.match(calls.notifications[0].body, /reopen automatically/i);
